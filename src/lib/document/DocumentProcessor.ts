...
const CHUNK_SIZE = 1000; // Number of tokens per chunk
...
export class DocumentProcessor {
  private words: Map<string, WordData>;
  private processingQueue: DocumentChunk[];
  ...
  private addWordTransition(currentWord: string, nextWord: string): void {
  ...
  private getNextWordProbabilities(currentWord: string): TransitionProbability[] {
    const wordData = this.words.get(currentWord);
    if (!wordData) return [];
  ...
  private async createChunks(text: string): Promise<DocumentChunk[]> {
    ...
    for (const sentence of sentences) {
      ...
      if (this.countTokens(potentialChunk) > CHUNK_SIZE && currentChunk) {
        chunks.push({
